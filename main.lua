...
local v15 = v14:CreateWindow({
    Title = "zuun", SubTitle = "Premium Script", TabWidth = 160, Theme = "Dark", Acrylic = false,
    Size = UDim2.fromOffset(500, 320), MinimizeKey = Enum.KeyCode.End
});
local v16 = {
...
-- Calculate distances from the current position to the two coordinates
local distance1 = (currentPos - pos1).Magnitude
local distance2 = (currentPos - pos2).Magnitude

-- Compare and select the closest coordinate for teleporting
local targetPos = if distance1 < distance2 then pos1 else pos2

-- Teleport to the closest coordinate (run once on start)
character:SetPrimaryPartCFrame(CFrame.new(targetPos))

-- Assume there are remote events in ReplicatedStorage for casting and reeling the rod
local castEvent = RS:WaitForChild("CastRod") -- Adjust remote name based on the game
local reelEvent = RS:WaitForChild("Reel") -- Adjust remote name based on the game

-- Equip the rod (assuming the name is "Starter Rod", change if different)
...
local function simulateClickFast()
    local clickFastGui = player.PlayerGui:WaitForChild("Main"):FindFirstChild("ClickFastFrame") -- Adjust GUI path
    if clickFastGui and clickFastGui.Visible then
        for i = 1, 100 do
            mouse1click() -- Requires an executor like Synapse
            wait(0.01)
        end
    end
end

-- Function to cast the rod and handle the process
...
-- Add toggle to Setting tab
v16.Setting:AddToggle("AutoFarm", {
    Title = "Auto Farm",
